...
class Germinados extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
          ...
          {this.props.data.map((product, index) => {
            if (product.name) {
              return (
                <div key={product._id}>
                  <Link
                    href="/productos/verduras-y-hortalizas/germinados/[id]"
                    as={`/productos/verduras-y-hortalizas/germinados/${product.urlRoute}`}
                  >
...
export async function getStaticProps() {
  ...
  let dataMapped = () => {
    return [...data.productos]
      .map((product) => {
        if (product.subcategory === "germinados") {
          return {
            _id: product._id,
            name: product.name,
            price: product.price,
