...
class Patatas extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
      ...
      <>
        <div className="metaDiv">
          <div>
            <h1>🥔Patatas🥔</h1>
            <p>
             Perfectas para freir , estofados y salsas.
            </p>
            <p>
          ...
          {this.props.data.map((product, index) => {
            if (product.name) {
              return (
                <div key={product._id}>
                  <Link
                    href="/productos/verduras-y-hortalizas/patatas/[id]"
                    as={`/productos/verduras-y-hortalizas/patatas/${product.urlRoute}`}
                  >
...
export async function getStaticProps() {
  ...
  let dataMapped = () => {
    return [...data.productos]
      .map((product) => {
        if (product.subcategory === "patatas") {
          return {
            _id: product._id,
            name: product.name,
            price: product.price,
