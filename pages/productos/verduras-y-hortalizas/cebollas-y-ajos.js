...
class CebollasYajos extends React.Component {
  constructor(props) {
    super(props);
    this.myVar = "";
    this.myFunction = () => {
...
componentWillUnmount(){
  ...
  render() {
    return (
      <>
        <Head>
          <title>🧅Cebollas y ajos🧄</title>
          <meta
            name="description"
            content="Dale un toque a tu cocina con estos ajos y cebollas! Escoge desde medio kilo!😉"
          ></meta>
        </Head>

        <div className="metaDiv">
          <div>
            <h1>🧅Cebollas y ajos🧄</h1>
            <p>
            Dale un toque a tu cocina con estos ajos y cebollas! Escoge desde medio kilo!😉
            </p>
            <p>
...
export async function getStaticProps() {
  ...
  let dataMapped = () => {
    return [...data.productos]
      .map((product) => {
        if (product.subcategory === "cebollas-y-ajos") {
          return {
            _id: product._id,
            name: product.name,
            price: product.price,
