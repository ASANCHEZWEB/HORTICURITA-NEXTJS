...
class SetasyChampinones extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
    ...
    return (
      <>
        <div className="metaDiv">
          <div>
            <h1>Setas y champiñones</h1>
            <p>
             Bajas en calorías y perfectas para guisar🍳
            </p>
            <p>
          ...
          {this.props.data.map((product, index) => {
            if (product.name) {
              return (
                <div key={product._id}>
                  <Link
                    href="/productos/verduras-y-hortalizas/setas-y-champinones/[id]"
                    as={`/productos/verduras-y-hortalizas/setas-y-champinones/${product.urlRoute}`}
                  >
...
export async function getStaticProps() {
  ...
  let dataMapped = () => {
    return [...data.productos]
      .map((product) => {
        if (product.subcategory === "setas y champiñones") {
          return {
            _id: product._id,
            name: product.name,
            price: product.price,
...
export default SetasyChampinones;
